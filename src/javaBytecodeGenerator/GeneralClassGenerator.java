...
public class GeneralClassGenerator extends ClassGen {

	/**
	 * This is a utility class of the BCEL library that helps us generate
	 * complex Java bytecodes, or bytecodes that have different forms depending
	 * on the type of their operands, or require to store constants in the
	 * constant pool. We might generate them without this helper object, but it
	...
	/**
	 * Yields the instruction factory that can be used to create complex Java
	 * bytecodes for this class generator.
	 *
	 * @return the factory
	 */
	 ...
	 * blocks reachable from it. It calls
	 * {@link #generateJavaBytecodeFollows(Block, Map, InstructionList)} and
	 * then {@link #removeRedundancies(InstructionList)}.
	 *
	 * @param block
	 *            the code from which the generation starts
	 * @return the Java bytecode for {@code block} and all blocks reachable from
	 *         it
	 */
	...
	/**
	 * Auxiliary method that generates the Java bytecode for the given block of
	 * code and for all blocks reachable from it. It uses a set of processed
	 * blocks in order to avoid looping.
	 *
	 * @param block
	 *            the block from which the code generation starts
	 * @param done
	 *            the set of blocks which have been already processed
	 * @param instructions
	 *            the Java bytecode that has already been generated. It gets
	 *            modified in order to include the Java bytecode generated for
	 *            {@code block} and for those that are reachable from it
	 * @return a reference to the first instruction of the Java bytecode
	 *         generated for this block
	 */

	private InstructionHandle generateJavaBytecode(Block block, Map<Block, InstructionHandle> done,
		...
		// we record the beginning of the Java bytecode generated for the block,
		// for future lookup
	...
	/**
	 * Auxiliary method that generates the Java bytecode for the blocks that
	 * follow a given block. That Java bytecode might include some <i>glue</i>,
	 * such as the conditional Java bytecode for the branching code blocks.
	 *
	 * @param block
	 *            the block for whose followers the code is being generated
	 * @param done
	 *            the set of blocks which have been already processed
	 * @param instructions
	 *            the Java bytecode that has already been generated from the
	 *            predecessors and from the code of {@code block}. It gets
	 *            modified in order to include the Java bytecode generated for
	 *            the followers of {@code block}, including some <i>glue</i>
	 */

	private void generateJavaBytecodeFollows(Block block, Map<Block, InstructionHandle> done,
			InstructionList instructions) {
		...
		if (!follows.isEmpty())
			if (follows.get(0).getBytecode().getHead() instanceof BranchingBytecode) {
				// we are facing a branch due to a comparison bytecode. That
				// bytecode
				// and its negation are at the beginning of our two following
				// blocks
				...
				// in between, we put some code that jumps to yesH if condition
				// holds, and to noH otherwise
				...
				// in between, we put a goto bytecode. Note that we need it
				// since we have no guarantee
				// that the code for follows will be appended exactly after that
				// for the block. The
				// follows blocks might indeed have been already translated into
				// Java bytecode, and hence
	 ...
	 *
	 * @param il
	 *            the Java bytecode which must be simplified
